<?php
    ...
    if($_POST){
        ...
        if(($_POST['txtemail']=="codernaf" && ($_POST['txtpassword']=="1812"))){
           $validate = '1';
           $_SESSION['txtemail']='codernaf';
        }
        // si no aplicamops un loader, facilmente solo usamos estas dos lineas: -->
        // sleep(2);
...
<body>
    ...
    <?php if($_POST && $validate != 1){ ?>
        <span> <?php echo 'error al iniciar seción' ?> </span>
    <?php }; ?>

    <section class="formContainer">
        <form class="form" action="login.php" method="post">
            <!-- loader -->
            <?php if($_POST && $validate == 1){ include 'loader.php' ?>
                <script>
                    setTimeout(function() {
                    window.location.href = "index.php"; // Cambia a la URL de destino
                    }, 3000); // 3 segundos
                </script>
            <?php }; ?>
            <!-- content form -->
            <div class="title">¡Hola!,<br><p>ingresa tus datos para iniciar seción.</p></div>
            <input class="input" name="txtemail" placeholder="Email o Username" value="<?php echo isset($_POST['txtemail']) ? $_POST['txtemail'] : ''; ?>" >
            <input class="input" name="txtpassword" placeholder="Contraseña" type="password" value="<?php echo isset($_POST['txtpassword']) ? $_POST['txtpassword'] : ''; ?>">
